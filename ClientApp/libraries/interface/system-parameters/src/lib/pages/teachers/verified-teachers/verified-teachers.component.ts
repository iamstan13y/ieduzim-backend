...
@Component({
  selector: 'app-verified-teachers',
  templateUrl: './verified-teachers.component.html',
  styleUrls: []
})
export class VerifiedTeachersComponent implements OnInit {
  ...
  getTeachers(e?) {
    this.teacher = null;
    this.service.getPaginated({ pageNumber: this.index, pageSize: 10 }, 'Teachers/verified').subscribe(res => {
      if (res.succeeded) {
        this.teachers = res;
      }
