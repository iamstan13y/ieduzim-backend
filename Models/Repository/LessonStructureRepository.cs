...
namespace IEduZimAPI.Models.Repository
{
    public class LessonStructureRepository : ILessonStructureRepository
    {
        ...
        public Task<Paginator<LessonStructure>> GetPagedByUserId(PageRequest request, int teacherId)
        {
            if (request == null) request = new PageRequest() { PageNumber = 1, PageSize = 10 };
            var req = _context.LessonStructures.Include(i => i.Subject).Include(c => c.Level).Include(x => x.Teacher).Where(a => a.TeacherId.Equals(teacherId)).ToList();
